...
function validateClientBrowserDimensions() {
  if (window.innerHeight < 500 || window.innerWidth < 800) {
    ...
    const inadequateClientResourcesOverlay = document.getElementById(
      "inadequate-client-resources-overlay"
    );
    if (inadequateClientResourcesOverlay.style.display === "block") return;
    const inadequateClientResourcesOverlayText = document.getElementById(
      "inadequate-client-resources-overlay-text"
    );
    inadequateClientResourcesOverlayText.innerHTML =
      "Minimum window size to use LabelPal is 500 x 800 px";
